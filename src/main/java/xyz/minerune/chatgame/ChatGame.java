...
public class ChatGame extends PluginBase {
    ...
    public void onEnable() {
        this.getServer().getScheduler().scheduleRepeatingTask(new GameTask(this, Arrays.asList(GameType.FLASH, GameType.DECODE, GameType.MATH, GameType.HANGMAN)), 20 * 60 * 7);
    ...
    public void flash() {
        ...
        for (Player player : this.getServer().getOnlinePlayers().values()) {
            player.sendMessage(Message.clean("§e", "\uE103 Первым отправьте строку %0 в чат и получите %1$", result, String.valueOf(reward)));
        }
    }
    ...
    public void decode() {
        ...
        for (Player player : this.getServer().getOnlinePlayers().values()) {
            player.sendMessage(Message.clean("§e", "\uE103 Первым составьте слово из заданных букв %0 и получите %1$", shuffled, String.valueOf(reward)));
        }
    }
    ...
    public void math() {
        ...
        for (Player player : this.getServer().getOnlinePlayers().values()) {
            player.sendMessage(Message.clean("§e", "\uE103 Первым решите пример %0 и получите %1$", example, String.valueOf(reward)));
        }
    }
    ...
    public void hangman() {
        ...
        for (Player player : this.getServer().getOnlinePlayers().values()) {
            ...
            player.sendMessage(Message.clean("§e", "\uE103 Первым расшифруйте слово %0 и получите %1$", changed, String.valueOf(reward)));
        }
    }
    ...
    public boolean ifCorrect(String var1) {
        if (this.getCurrent() != GameType.NONE) {
            String var2 = var1.replace(" ", "");
            var2 = TextFormat.clean(var2);

            String var3 = getResult();
            String var5 = var3.replace(" ", "");

            return var2.equalsIgnoreCase(var5);
        }
        return false;
    }
